...
def create_dataset(X_train,y_train,batch_size=16,repeat_size=1):
    ...
    创建数据迭代器
    '''
    input_data=ds.GeneratorDataset(list(get_train(X_train,y_train)),column_names=['data','label'])
...
def main():
    # ===================================================
    # 读入数据->绝对路径
    df = pd.read_csv("F:/junior/xia/ML/实验-mindspore/Chapter-2/波士顿房价预测/boston.csv")
